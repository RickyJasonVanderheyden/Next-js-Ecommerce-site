...
import Link from 'next/link';  //anchor tag is not used because it refreshes the page, uncessary refresh. higher performance link
...
export default function ProductsList({ products, initialCartProducts }: { products: Product[], initialCartProducts: Product[] }) {
  ...
  async function addToCart(productId: String) {
    ...
    const uniqueCartProducts = updatedCartProducts.filter((product: Product, index: number, self: Product[]) => 
      index === self.findIndex(p => p.id === product.id)
    );
    
    setCartProducts(uniqueCartProducts); // This should update the UI immediately
  }
  ...
  async function removeFromCart(productId: String) {
    ...
    setCartProducts(uniqueCartProducts);
  }
